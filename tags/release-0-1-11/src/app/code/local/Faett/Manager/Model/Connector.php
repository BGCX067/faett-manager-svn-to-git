...
class Faett_Manager_Model_Connector
    ...
    public function validate(
        ...
        if ($data = $sess->getData($key)) {
	        // log that a valid serialz was found in the session
        	Mage::log('Found session information for package ' . $key . '!');
    		// check if a valid licence was found
	        if (array_key_exists(self::HAS_VALID_LICENCE, $data) && 
	            $data[self::HAS_VALID_LICENCE]) {
	            // log that a valid serialz for the package was found in the session
	           	Mage::log('Found valid serialz in session!');
	        	// if yes, return without doing anything
	        	return;
	        }
        }
        // log that the serialz will now be validated
        Mage::log('Now validating serialz!');
        ...
        // log a message that login was successfully
        Mage::log('Successfully logged in to channel ' . $channelUrl);
        ...
        if ($isValid) {
        	Mage::log('Now adding information for package ' . $key . ' to session!');
