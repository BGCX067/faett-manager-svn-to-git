...
class Faett_Manager_Model_Connector
    ...
    /**
     * Validation method for SOAP call.
     * @var string
     */
    const CHANNEL_VALIDATE_SIMPLE = 'channel.validateSimple';
    ...
    public function validate(
        ...
        try {
            ...
            $client->call(
                $session,
                Faett_Manager_Model_Connector::CHANNEL_VALIDATE_SIMPLE,
                array(
            ...
            throw Faett_Manager_Exceptions_InvalidLicenceException::create(
            	'Licence is not valid for package ' . $information->getIdentifier()
            );           
        }
        // if a valid licence was found set it in the session
        $sess->setData(
            $key, 
            array(self::HAS_VALID_LICENCE => true)
        );
    }
